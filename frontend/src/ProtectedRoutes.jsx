...
function ProtectedRoutes({ fileData, children, path }) {
  // Change to false to activate protected routes
  const [isChecksComplete, setIsChecksComplete] = useState(true);
  const navigate = useNavigate();

  const allowedPathToNotHaveKey = ["login", "register", "hero"];
  const allowedPathToNotHaveData = [...allowedPathToNotHaveKey, "upload"];

  // Uncomment the whole useEffect to activate protected routes
  // useEffect(() => {
